...
import type { FormErrors } from '../../types/bids'

describe('BidCalculatorForm.vue', () => {
  const defaultProps = {
    formData: {
      basePrice: 1000,
      carType: 'Common' as const
    },
    isSubmitting: false,
    errors: {},
    errorMessage: ''
  ...
  describe('Component Rendering', () => {
    ...
    it('renders car type options correctly', () => {
      ...
      expect(options[0].attributes('value')).toBe('Common')
      expect(options[1].text()).toBe('Luxury')
      expect(options[1].attributes('value')).toBe('Luxury')
    })
  })
  ...
  describe('Props Display', () => {
    ...
    it('displays the car type value from props', () => {
      const wrapper = createWrapper({
        formData: { basePrice: 1000, carType: 'Luxury' }
      })
      
      const select = wrapper.find('select#carType')
      expect((select.element as HTMLSelectElement).value).toBe('Luxury')
    })
  ...
  describe('Event Emission Details', () => {
    ...
    it('emits update event when car type selection changes', async () => {
      ...
      await select.setValue('Luxury')
      
      expect(wrapper.emitted('update')).toBeTruthy()
      expect(wrapper.emitted('update')?.[0]).toEqual(['carType', 'Luxury'])
    })
    ...
    it('emits multiple update events for sequential changes', async () => {
      ...
      await wrapper.find('select#carType').setValue('Luxury')
      ...
      expect(updateEvents?.[1]).toEqual(['carType', 'Luxury'])
    ...
    it('emits correct data types for different fields', async () => {
      ...
      await wrapper.find('select#carType').setValue('Common')
      
      const updateEvents = wrapper.emitted('update')
      expect(updateEvents?.[0]).toEqual(['basePrice', 1500]) // number
      expect(updateEvents?.[1]).toEqual(['carType', 'Common']) // string
    })
  })
